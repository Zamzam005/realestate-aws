...
function validateMessage(input) {
  ...
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || email.length > 120) {
    errors.push("email must be a valid address up to 120 characters");
  }
...
export async function putMessage(input) {
  const { name, email, message } = validateMessage(input);
  ...
  await ddb.send(new PutCommand({
    TableName: process.env.MESSAGES_TABLE || "RealEstate-Messages",
    Item: item,
  }));
